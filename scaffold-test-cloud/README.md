...
使用服务发现的示例应先启动 Nacos。推荐使用本模块的统一基础设施目录：
...
默认 Nacos 使用内置 Derby。若要使用 MySQL 持久化，可在 `container` 目录使用：
...
使用 [`container`](container) 启动基础设施时，会自动将
[`container/seata/seataServer.properties`](container/seata/seataServer.properties)
...
同步更新 [`container/seata/application.yml`](container/seata/application.yml)。
...
```bash
./scripts/cloud-compose.sh ps
# 或 docker compose -f docker-compose.cloud.yml port cloud-gateway 10000
```
...
```bash
./scripts/cloud-compose.sh logs cloud-provider
./scripts/cloud-compose.sh down
```
...
[`container/.env`](container/.env) 中覆盖端口或账号：
...
[`container/prometheus/prometheus.yml`](container/prometheus/prometheus.yml)。
