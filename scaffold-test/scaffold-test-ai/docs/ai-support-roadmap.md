...
- [x] 已完成阶段四多轮对话，并将完整消息持久化到 PostgreSQL，重启后可重建模型会话窗口。
- [x] 已进入阶段五，知识文档持久化到 PostgreSQL，并提供来源引用和无依据拒答。
...
当前使用 `scaffold-orm-starter`、Spring Data JPA 和 PostgreSQL。`WorkOrderEntity` 继承
`BaseAuditable`，数据库通过 `(user_id, request_id)` 唯一约束保护幂等语义。演示环境允许 Hibernate
自动更新表结构；进入生产前应使用 Flyway 或 Liquibase 管理 Schema。
...
- [x] `ToolContext` 中包含服务端提供的用户和 requestId。
- [x] 模型不能传入或覆盖 userId。
...
直接使用 PostgreSQL + pgvector 跑通文档加载、切分、Embedding、检索和回答，并保证知识文档与向量索引持久化。
回答必须附带来源，没有可靠片段时明确拒答。

完成标准：

- [x] 文档具有 documentId、版本和更新时间。
- [x] `KnowledgeAnswer` 包含答案和引用来源。
- [x] 没有相关资料时不允许模型凭常识猜测。
- [x] 文档更新和删除后可以同步处理向量。
- [ ] 测试正确引用和无依据拒答。
...
- [x] 建议结果为受校验的结构化对象。
- [x] 事实性结论可以追溯到订单数据或知识来源。
- [x] 建议不会直接改变工单、订单或退款状态。
- [x] 高风险和信息不足的建议自动标记人工审核。
- [x] 前端同时展示建议、依据和风险提示。
...
- [x] confirmationId 与当前用户和具体操作参数绑定。
- [x] confirmationId 短期有效、只能使用一次且不可预测。
- [x] 确认时重新检查权限、订单状态、金额和业务规则。
- [x] 最终执行具备幂等能力。
- [x] 取消、过期、重复确认和越权确认都有测试。
- [x] 所有准备、确认和执行动作进入审计日志。
...
阶段一至四已经完成。阶段五已使用本地中文 ONNX Embedding 与 pgvector 跑通文档切分、向量持久化、
Top-K 检索、来源引用、无依据拒答和索引重建。
