...
public class TreeUtils {
    ...
    public static <T> List<ITree<T>> tree2(List<ITree<T>> list, T rootId) {
        Map<Optional<T>, List<ITree<T>>> map = list.stream().collect(Collectors.groupingBy(tiTree -> Optional.ofNullable(tiTree.getParentId())));
        return parseChild(map, rootId);
    }

    public static <T> List<ITree<T>> parseChild(Map<Optional<T>, List<ITree<T>>> map, T rootId) {
        List<ITree<T>> list = map.remove(Optional.ofNullable(rootId));
