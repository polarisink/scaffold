...
public final class RedisUtils {
     ...
     * @return 组
     */
    public static StreamInfo.XInfoGroups groups(String key) {
     ...
     * @return
     */
    public static Long acknowledge(String group, Record record) {
     ...
     * @return 记录id
     */
    public static RecordId streamAdd(String streamKey, String object) {
        ObjectRecord<String, String> record = StreamRecords.objectBacked(object).withStreamKey(streamKey);
    ...
    public static void del(String... keys) {
        if (keys != null && keys.length > 0) {
            if (keys.length == 1) {
                ...
                log.debug("--------------------------------------------");
                log.debug(new StringBuffer("删除缓存：").append(keys[0]).append("，结果：").append(result).toString());
                log.debug("--------------------------------------------");
            } else {
    ...
    public static List<String> multiGet(List<String> keys) {
        List list = redisTemplate.opsForValue().multiGet(keys.stream().collect(Collectors.toSet()));
    ...
    public static void set(String key, Object value, Long time, TimeUnit timeUnit) {
        redisTemplate.opsForValue().set(key, JsonUtil.toRedisJson(value), time, timeUnit);
    }

    public static void setWithLogicalExpire(String key, Object value, Long time, TimeUnit timeUnit) {
        RedisData redisData = new RedisData();
        redisData.setData(value);
        redisData.setExpireTime(LocalDateTime.now().plusSeconds(timeUnit.toSeconds(time)));
        redisTemplate.opsForValue().set(key, JsonUtil.toRedisJson(redisData), time, timeUnit);
    }

    /**
     * 普通数据 查询缓存  null  缓存空值  需要限流不然 第一次 全部都是查询null
     *
     ...
     * @return {@link R}
     */
    public static <R, ID> R queryCache(String keyPrefix, ID id, Class<R> resultType, Long time, TimeUnit timeUnit, Function<ID, R> dbFallback) {
        ...
        log.info("缓存中 数据真的是不存在");
        // 查询数据库
        R r = dbFallback.apply(id);
        if (null == r) {
            // 查询出来的结果是null 缓存空值
            log.info("数据库也没有查询到数据");
    ...
    /**
     * 计算更新缓存,不更新时updateFallback传入Function.identity()
     * <p>
     * 如果缓存存在，更新缓存
     * 如果缓存不存在，加入缓存
     *
     ...
     * @param dbFallback     缓存不存在时从数据库查询操作
     * @param updateFallback 缓存存在时更新操作,如果只查询不更新就传入Function.identity()
     * @param <R>            结果类型
     * @param <ID>           id类型
     * @return
     */
    public static <R, ID> R compute(String keyPrefix, ID id, TypeReference<R> resultType, Long time, TimeUnit timeUnit, Function<ID, R> dbFallback, Function<R, R> updateFallback) {
        String key = keyPrefix + id;
        String json = get(key);
        R r;
        if (null != json && !json.isEmpty()) {
            ...
            log.info("【{}】命中缓存", key);
            //转换缓存
            r = JsonUtil.redisRead(json, resultType);
            //缓存操作有用才处理,否则就是读操作，直接返回即可
            if (!Objects.equals(updateFallback, Function.identity())) {
                r = updateFallback.apply(r);
                set(key, r, time, timeUnit);
            }
            return r;
        }
        if (json != null) {//说明缓存的"",empty字符串
            log.info("【{}】缓存的空字符串", key);
            return null;
        }
        log.info("【{}】不存在缓存数据", key);
        //缓存没有，查数据库
        if ((r = dbFallback.apply(id)) == null) {
            log.info("数据库中也没有【{}】对应数据", key);
            //查询也没查到,设置空值，解决穿透
            set(key, "", 20, TimeUnit.SECONDS);
            return null;
        }
        set(key, r, time, timeUnit);
        return r;
