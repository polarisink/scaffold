...
public class PostgresqlCacheStore {
    ...
    public void initializeSchema(String tableName, boolean unlogged) {
        ...
        jdbcTemplate.execute(
                "CREATE INDEX IF NOT EXISTS " + indexName(validatedTableName) + " ON " + validatedTableName + " (expires_at)"
        );
    }
    ...
    public List<byte[]> findValue(String tableName, String cacheName, String cacheKey) {
        ...
        return jdbcTemplate.query(
                "SELECT cache_value FROM %s WHERE cache_name = ? AND cache_key = ? AND (expires_at IS NULL OR expires_at > CURRENT_TIMESTAMP) LIMIT 1".formatted(validatedTableName),
    ...
    public void evictExpired(String tableName, String cacheName, String cacheKey) {
        String validatedTableName = validateTableName(tableName);
        jdbcTemplate.update(
                "DELETE FROM %s WHERE cache_name = ? AND cache_key = ? AND expires_at IS NOT NULL AND expires_at <= CURRENT_TIMESTAMP".formatted(validatedTableName),
                cacheName,
                cacheKey
        );
    }
